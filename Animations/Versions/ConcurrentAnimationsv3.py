#!/usr/bin/env python
"""
All animations require same size slavedrivers and all their buffers are xor'd
"""
import threading
...
if __name__ == '__main__':  
    ...
    # to the driver
    ledslaves = [LEDStrip(DriverSlave(160), threadedUpdate=True) for _ in range(len(wormdatalist))]
    
    # Make the Worm animations an list pairs (animation, fps)
    wormlist = [(Worm(ledslaves[i], *d[0]), d[1]) for i, d in enumerate(wormdatalist)]
    
    ledslaveb = LEDMatrix(DriverSlave(160), width=16, height=10,  threadedUpdate=True)
